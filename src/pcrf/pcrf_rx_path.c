                          ...
                          struct session *sess, void *opaque, enum disp_action *act)
{
    ...
    if (avp)
    {
        ...
        d_assert(ret == 0, return EINVAL, );
        printf("found sid: %s", (os0_t)pcrf_sess_find_by_ipv4(hdr->avp_value->os.data));
        unsigned char bytes[4];
        bytes[0] = (uint8_t) hdr->avp_value->os.data & 0xFF;
        bytes[1] = (uint8_t) ((hdr->avp_value->os.data) >> 8) & 0xFF;
        bytes[2] = (uint8_t) ((hdr->avp_value->os.data) >> 16) & 0xFF;
        bytes[3] = (uint8_t) ((hdr->avp_value->os.data) >> 24) & 0xFF;
    ...
    while (avpch1)
    {
        ...
        {
            rx_media_component_t *media_component = &rx_message.media_component[rx_message.num_of_media_component];
            ...
            while (avpch2)
            {
                ...
                {
                    rx_media_sub_component_t *sub = &media_component->sub[media_component->num_of_sub];
